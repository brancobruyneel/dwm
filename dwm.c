...
deck(Monitor *m) {
	...
	for(i = my = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if(i < m->nmaster) {
			...
			resize(c, m->wx + mw, m->wy, m->ww - mw - (2*c->bw), m->wh - (2*c->bw), c->bw, 0);
}
...
bstack(Monitor *m) {
	...
	for (i = mx = 0, tx = m->wx, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++) {
		if (i < m->nmaster) {
			...
			resize(c, m->wx + mx, m->wy, w - (2 * c->bw), mh - (2 * c->bw), c->bw, 0);
			mx += WIDTH(c);
		} else {
			h = m->wh - mh;
			resize(c, tx, ty, tw - (2 * c->bw), h - (2 * c->bw), c->bw, 0);
			if (tw != m->ww)
				tx += WIDTH(c);
		}
	}
...
bstackhoriz(Monitor *m) {
	...
	for (i = mx = 0, tx = m->wx, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++) {
		if (i < m->nmaster) {
			...
			resize(c, tx, ty, m->ww - (2 * c->bw), th - (2 * c->bw), c->bw, 0);
			if (th != m->wh)
				ty += HEIGHT(c);
		}
	}
